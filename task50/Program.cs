...
void PosValueArray(int[,] array)
{
      if (m >= r || n >= c) Console.Write($"Такого числа в массиве нет");
                
      if (m < r && n < c) Console.Write($"Элемент с позицией [{m}, {n}]  {array[m, n]}");
                    
}

int[,] myArray = GetArray(r, c, minValue, maxValue);
PrintArray(myArray);
Console.WriteLine();
